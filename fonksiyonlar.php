...
function fnk(){
	echo "<script language=javascript>document.write(unescape('<center><h3>Yiğit GÜLAY ADINA YAPILMIŞTIR.</h3><center>'))</script>
	";
}

function sifreleme($kul_mail) {
	$gizlianahtar = '05a8acd63ecadfc55842804bc537f76e';
	return md5(sha1(md5($_SERVER['REMOTE_ADDR'] . $gizlianahtar . $kul_mail . "Yigitgly" . date('d.m.Y H:i:s') . $_SERVER['HTTP_USER_AGENT'])));
};
